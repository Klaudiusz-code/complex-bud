...
const icons = [
  ...
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75h-4.5m4.5 0v4.5m-4.5-4.5L15 9m5.25 11.25h-4.5m4.5 0v-4.5m0 4.5L15 15"
    />
  </svg>,
];

const Pillars = ({ data }: PillarsProps) => {
  const pillars = data?.filary ?? [];
        ...
        <div className="mb-12 sm:mb-16">
          ...
          <div className="mt-4 flex items-center gap-6 lg:gap-10">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-neutral-900 dark:text-white whitespace-nowrap transition-colors">
              {data?.naglowekSekcji || "Fundamenty naszej pracy"}
            </h2>

            <div className="hidden sm:block h-px flex-1 bg-neutral-200 dark:bg-white/[0.06]" />
          </div>
        </div>
